...
\twocolumn[
  \begin{@twocolumnfalse}
    ...
    % Motivation
    Performance analysis in computer science is prone to measurement bias, where seemingly innocious properties of the execution context, such as the size of system environment variables, can impact performance in non-obvious ways.
    Measurement bias has been shown to be a significant, but unpredictable and difficult to deal with.
    In order to get reliable data for potentially biased programs, it might be necessary to average experiments over multiple environment configurations.

    % Problem statement
    Our goal is to identify some of the underlying mechanisms in the Intel ``Haswell'' processor architecture that causes measurement bias, and see how that can be accounted for in an attempt at optimization.

    % Approach
    Starting with the high level known causes of bias from previous work, we analyze a minimal C-program that exhibits bias from environment size.
    In order to understand why performance varies, we use hardware performance counters to obtain detailed statistics for different environments.

    % Results
    We find that an effect known as 4K aliasing, causing resource stalls in the CPU execution pipeline, can be a common source of bias from external factors that alters run time addresses of program data.
    For some applications, performance penalties from unfavorable memory address allocations can be as high as double the cycle count.
    For ATLAS, an auto-tuned BLAS implementation, we estimate a potential 30\% performance variation in one matrix-vector multiplication routine, depending on the location in memory of each buffer.

    % Conclusions
    With an understanding of 4K aliasing, we show that bias is in fact not completely unpredictable, and discuss measures for avoiding bad performance because of it.
    \end{abstract}
    \bigskip
...
Like the contents of environment variables, the exact operation of heap allocation can typically not be known ahead of time.
...
\begin{table*}[t]
  ...
  \begin{tabular}{l r r r r}
    \toprule
      & ptmalloc & tcmalloc & jemalloc & Hoard \\
    \midrule
    64 B       & 0x602010 & 0xe0e000 & 0x2aaaabc0e040 & 0x2aaaaab20070 \\
               & 0x602060 & 0xe0e080 & 0x2aaaabc0e080 & 0x2aaaaab200b0 \\
    \midrule
    1024 B     & 0x6020b0 & 0xe12000 & 0x2aaaabc0f400 & 0x2aaaaab30070 \\
               & 0x6024c0 & 0xe12800 & 0x2aaaabc0f800 & 0x2aaaaab30470 \\
    \midrule
    5120 B     & 0x6028d0 & 0xe16000 & 0x2aaaabc1f000 & 0x2aaaaab40070 \\
               & 0x603ce0 & 0xe18800 & 0x2aaaabc21000 & 0x2aaaaab42070 \\
    \midrule
    $2^{20}$ B & 0x2aaaaaaf3010 & 0xe64000 & 0x2aaaabc86000 & 0x2aaaabf40070 \\
               & 0x2aaaab098010 & 0xf64000 & 0x2aaaabd86000 & 0x2aaaac060070 \\
    \bottomrule
  \end{tabular}
\end{table*}
