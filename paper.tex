...
Recent research has shown that there can be significant performance differences based on seemingly irrelevant factors like user name length.
...
In the longer term, our results can enable novel compiler and architectural optimization strategies.
...
This behavior is often the worst case for functions that operate on two or more independent buffers, as we will see in the following section.
...
There is one example where an allocator yields aliasing buffers while another does not.
