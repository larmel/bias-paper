...
Sem S{\ae}lands vei 9, Gl{\o}shaugen, 7491 Trondheim, Norway; email: runeerle@idi.ntnu.no
%LARS: din addresse er ok (NTNU)
\end{bottomstuff}
...
The \texttt{cycles_ldm_pending} and \texttt{stalls_l2_pending} are interesting as they are related to the memory and cache system.
...
\end{lstlisting}
An estimate of the actual cost a single invocation can be calculated by averaging over a number of repeated function calls, after subtracting the constant overhead of invoking it only once.
\[
...
This phenomenon is only observed for address deltas close to zero, thus we only show the first 20 data points.
...
Unlike the microkernel in \ref{sec:microkernel} the execution and RS stall counts drop with together with aliasing events.
Speculation is possible in this kernel as there are many independent store addresses.
False aliasing limits this speculation however, and makes the CPU incorrectly discard executed instructions.
This leads to the inverted behavior compared to the microkernel.

It is worth noticing that most cache related metrics does \emph{not} stand out in this experiment.
Again the \texttt{cycles_ldm_pending} and \texttt{stalls_l2_pending} behavior is the same as in the microkernel in \ref{sec:microkernel}.
...
%his goes somewhat against conventional wisdom that more alignment is better.
...
We have shown how address aliasing can affect program performance under different memory layouts, and how it can explain certain cases of measurement bias. 
The effect is caused by how speculative and out-of-order memory operations are handled by the CPU, which only considers the last 12 address bits to resolve conflicts between load and store operations.
...
We show how aliasing can have significant performance impact in practice for algorithms operating on heap allocated buffers, analyzing an example with more than $2x$ performance variation between different memory layouts.
Heap memory is controlled by allocators, and we show that most libraries tend to favor page alignment for larger requests.
This means that pointers to structures such as arrays and matrices are likely to alias by default, which can be worst case input for many algorithms.
