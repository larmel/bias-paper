...
\twocolumn[
  \begin{@twocolumnfalse}
    \maketitle
    \begin{abstract}
    Seemingly innocuous properties of the execution context, such as the contents of system environment variables, can impact the performance of computer programs.
    Factors impacting performance that are not directly related to the subject being measured can introduce bias towards certain configurations.
    Measurement bias has been shown to be a significant issue in performance analysis, appearing to be unpredictable and difficult to deal with.

    This paper focuses on the underlying reasons for bias effects that can be experienced for example by changing environment variables or using different dynamic allocation libraries.
    Each alteration of the environment we look at has the effect of changing run time addresses of data or code.
    Through experimentation and careful measurements using hardware performance counters, we identify that address aliasing on the low 12 bits between reads and writes can cause stalls in the execution pipeline.
    Depending on initial memory layout at program start, this can be a common source of bias.
    ...
    In a case study of auto-tuned BLAS implementation ATLAS, we estimate a potential 30\% performance variation in one matrix-vector multiplication routine, depending on the location in memory of each buffer.
    Note that these effects are not a result of cache issues, but a heuristic in the out of order engine found in recent Intel microarchitectures.

    With an understanding of 4K aliasing, we show that bias is in fact not completely unpredictable, and discuss measures for avoiding it.
    \end{abstract}
    \bigskip
