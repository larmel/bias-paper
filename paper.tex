...
In this paper we show how \emph{address aliasing} can often be the underlying mechanism resulting in measurement bias on modern Intel microarchitectures.
...
\begin{narrowfig}{0.5\textwidth}[t]
  ...
  The stack is placed close to the top, but offset by environment variables and program arguments. 
  Program code and statically allocated data is allocated at the lower end of the address space.
  ...
  % \footnote{Modern processors do not actually use the full 64-bit space, only the low order 47 bits are used for addressing memory}
...
% The stack is placed close to the upper address 0x7fff'ffffffff\footnote{Modern processors do not actually use the full 64-bit space, only the low order 47 bits are used for addressing memory}, but offset by environment variables and program arguments.
% After some variable amount of offset, the stack is normally aligned to a 16 byte boundary, which will be enforced by the compiler. % [ref]
...
% Note that there is no clear relationship between environment size and stack location with ASLR enabled. % todo: check again
...
%\begin{figure}
  ...
  % Can have caption with figure environment
  %\caption{Micro-kernel succeptible to bias from variations to system environment variables.\label{lst:loopkernel}}
%\end{figure}
...
\begin{figure}
  % [float=t, language=C, caption={Dynamically detect aliasing case, and avoid by pushing another stack frame.}, label={lst:loopfixed}, frame=lines]
  \begin{lstlisting}
  #define ALIAS(a, b) \
...
As an example of this, consider a naive implementation of linear correlation shown in Figure \ref{lst:conv} (for simplicity, endpoints are not handled).
...
\begin{table}[t]
  ...
  \tbl{Most interesting performance counters and correlation with cycle count for optimization O2, estimated cost accounting for constant overhead.}{
    \pgfplotstabletypeset[
      font=\footnotesize,
      int detect, % Output whole numbers for counter values
      col sep=comma,
