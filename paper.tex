...
On recent architectures such as Haswell and Ivy Bridge, the number of times this happens can be counted by the following performance counter:
\begin{description}
...
\end{description}
Older architectures such as Core2 does not have the same event available.
However, the effect is still described in the manual for these processors, and covered by the following event:
\begin{description}
  \item[{\small LOAD\_BLOCKS.OVERLAP\_STORE}] ``(...) Loads that partially overlap an earlier store, or 4-Kbyte aliased with a previous store.'' ~\citeN[Table 19-17]{Volume3B}
\end{description}
