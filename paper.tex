...
Understanding program performance through accurate measurements is important in performance analysis, but experiments can be \emph{biased} by external factors.
Seemingly irrelevant variables, such as the length of your user name, has been shown to potentially impact performance.
...
Our approach is to search for correlations between a large set of hardware performance counters from a controlled series of execution contexts.
...
For heap allocators, we show that common libraries tend to give worst case alignment by default, favoring page alignment for large allocations.
...
Our results show there is a need for new architecture specific optimization strategies to account for this phenomenon.
\end{abstract}
