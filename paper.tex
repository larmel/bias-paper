...
\begin{enumerate}
  ...
  %For larger allocations, it can be more efficient to use anonymous memory mapping by calling the Posix function \texttt{mmap} instead of growing the regular heap.
\end{enumerate}
% One drawback of using memory mapping is that it requires the buffer to be zero initialized, so which mechanism is used can be based on some cost heuristic employed by the allocator.
...
% The normal heap is often used for smaller requests, while memory mapping is used for large buffers.
