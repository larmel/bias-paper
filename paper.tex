...
%Understanding and accounting for external measurement \emph{bias} is important in all science.
Understanding program performance and obtaining accurate measurements is important in all scientific work.
Any source of \emph{bias} by external factors should therefore be known and accounted for.
However, recent research has shown that there can be significant performance differences based on seemingly irrelevant factors such as user name length.
In this paper we identify an important underlying mechanism that cause this type of measurement bias on modern Intel microarchitectures.
...
We demonstrate this by implementing runtime detection and correction of address aliasing, eliminating the bias.
Further, we show that the allocation logic of common libraries tends to give \emph{worst case} alignment by default, by favoring page alignment large allocations.
The performance impact of unfavorable memory layout combinations can be significant, with as much as $2x$ speedup in one of our examples.
...
In the longer term, our results can enable new architecture specific optimization strategies.
...
%This process is complicated by \emph{measurement bias}, which can be a challenge for performance analysist and systems researchers.
...
% A likely reason this happens is because certain variables has the potential to offset memory addresses of code or data, which in turn interacts with various low level hardware buffers at runtime.
% Because of the complexity of modern processors, it is often hard or even impossible to predict exactly how changes interact. %to things like system environment variables ultimately affect program execution.
The underlying explanation is likely that this changes memory addresses of code and/or data, which in turn interacts with various low level hardware buffers at runtime.
...
Using this technique avoids modifications to the executed program, unlike tools like PAPI [LARS cite: PAPI: A Portable Interface to Hardware Performance Counters].
...
Reliability of PMU measurements have been evaluated by Weaver and Sally in [Can hardware performance counters be trusted? 2008].
...
We do not use multiplexing between a limited set of counter registers, as it will introduce too many variations [Lars cite: Toward Accurate Performance Evaluation using Hardware Counters].
...
All experiments are done on a dedicated machine under minimal load, and \emph{frequency scaling} is disabled to keep the CPU's clock speed fixed.
...
After this offset, the stack is normally aligned to a 16 byte boundary, enforced by the compiler.
Within a span of 4096 bytes there are thus 256 possible initial stack addresses, each representing a different execution context with respect to address aliasing. Note that there is no clear relationship between environment size and stack location with ASLR enabled.
...
Going through the list of events manually, we narrowed down to a set that seem to correlate well with cycle count.
Table \ref{tab:loopcorrelation} shows numerical counter values for worst cases, compared to the median values.
...
While this will give significantly more alias counts, we found that it has little effect on the total number of cycles executed.
...
While this particular solution is clearly not something one would want to do in practice, it shows that compilers and programmers \emph{could} take measures to account for aliasing.
...
We see one example where an allocator yields aliasing buffers while another does not.
...
But even with randomization, pointers returned by \texttt{mmap} must still be page aligned.
...
As an example of this, consider a na{\"i}ve implementation of linear correlation shown in Figure \ref{lst:conv}.
...
To analyze performance for different addresses, we manually insert padding to offset one of the memory mapped pointers.
This is accomplished by requesting a bit more memory, and use pointer arithmetic to offset one of the function arguments.
Controlling the offset parameter, we can create environments where the address suffixes are some number of \texttt{sizeof(float)} bytes apart.
...
For every run, performance counter measurements are also averaged over 10 samples, using the repeat mechanism of perf.
...
\begin{figure}[t]
  ...
  \caption{Cycle- and alias counts for different offsets between input and output arrays in convolution kernel from Figure~\ref{lst:conv}. Offset 0 means equal 12-bit address suffix, which is default behavior for \texttt{mmap} allocations. Showing optimization levels O2 and O3, input size $n=2^{20}$.}
\end{figure}
...
This phenomenon is only observed for address delta close to zero, thus we only show the first 20 data points.
...
More detailed performance counter data is presented in Table~\ref{tab:convstats}, where we have selected a subset that correlates well with the change to cycle count.
The numbers are for the level 2 optimization case, which we choose to study in more detail.
 % which was chosen because the behavior was less chaotic and thus more easily represented by only a few numbers.
...
The L1 hit rate remains stable across all offsets, and only a negligible number of memory loads actually misses L1.
...
This goes somewhat against conventional wisdom that more alignment is better.
The Intel optimization manual actually mentions this in \emph{User/Source Coding Rule 8}, suggesting that special purpose allocators could be used to avoid aliasing ~\cite{OptimizationManual}.
...
Because typical heap allocators will return aliased pointers for large allocations, worst case performance is likely to be the default.
