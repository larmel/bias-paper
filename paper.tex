...
% TODO: Lars: make this a separate section or move it up a bit?
This type of bias can occur in a number of memory configurations.
Here the aliasing was caused by interactions between the stack and static variables.
The stack and heap can interact the same way, depending on how the malloc function behaves.
Since the stack is used for local variables complex code is likely contain many potential conflicts.
A evaluation of how malloc behaves is presented in \ref{sec:heap}.
...
We do not attempt to pinpoint exactly which assembly instructions might alias, but we assume that the processor thinks memory accesses to \texttt{input[i]} potentially conflicts with \texttt{output[i]}.
...
We show how aliasing can have significant performance impact for algorithms operating on heap allocated buffers, analyzing an example with more than $2x$ performance variation between different memory layouts.
Heap memory is controlled by allocators, and we show that most libraries tend to favor page alignment for larger requests.
This means structures such as vectors and matrices are likely to alias by default, which can be worst case scenarios for many algorithms.
