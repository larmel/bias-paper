...
Notably, this event was also found to be significant in the original paper by \citeN{Mytkowicz:2009:WrongData}, where the authors used a similar setup.
...
\begin{describe}{{pad}}
  ...
  \item[\emph{Level 3}.] Matrix-matrix operations, including the widely applied gemm routine for general matrix-matrix multiplication.
\end{describe}
Procedures operating on vectors, from level 1 or 2, intuitively appears the most likely to have potential for aliasing.

We look at three different libraries available from the Ubuntu package manager; \emph{libblas}, \emph{libatlas}, and \emph{libopenblas}.
We ran our bias analysis on the level 2 procedure \texttt{cblas\_dgemv}, which computes the matrix-vector product $\boldsymbol{y} = \alpha\text{op}\left(A\right)\boldsymbol{x} + \beta\boldsymbol{y}$
...
Out of trial and error, it turns out the memory address offset of vector $y$ compared to matrix $A$ is the most important with respect to the number of alias events.
For the following experiment, we set $A = I$ (identity matrix) and $x = [1, 2, ... N]$, with parameters $M = N = 8192$.
Address suffix of $A$ and $x$ is fixed to 0x000, while performance events are sampled for increasing address offset of $y$, starting at a baseline of 0x000.
The experiment was run on the ``Haswell'' setup, with Hyper-Threading enabled.
In an attempt to isolate the cost of the \texttt{cblas_dgemv} invocation, we use the estimation approach described in Section~\ref{sec:heap} to subtract the constant overhead from heap allocation and initialization.
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    \begin{axis}[
        ...
        ylabel=Event count,
        cycle list name=black white,
        width=\textwidth/2.7, % Make it fit to text width side by side
  ...
  \end{tikzpicture}
  \label{fig:blas}
  \caption{Performance of \texttt{dgemv} for different {\small{BLAS}} packages, with varying address offset between matrix $A$ and vector $y$.}
\end{figure}
...
The diagrams can be understood the same way as for the convolution example; partial address overlap occur when the address suffix delta of $A$ and $y$ is close to zero.
Libblas generates the most distinct pattern, with a relatively clear spike for deltas between 1 and 6 \texttt{double} elements.
% Need to run experiment again
Correlation between cycles executed and address aliasing events were calculated to 95\% for libblas, 78\% for libatlas and 44\% for libopenblas.
...
The amount of \emph{bias} this creates with respect to number of cycles executed is not as clear in all cases, but at least for the libblas case we are pretty confident in claiming that aliasing is indeed causing the performance degradation.
...
The microkernel example we analyzed is taken from a followup paper by the same authors, where it is used as an example of how environment size can cause bias~\cite{Mytkowicz:2009:WrongData}.
...
Finally, we look at different linear algebra libraries and find examples of alias issues in matrix-vector multiplication routines.
