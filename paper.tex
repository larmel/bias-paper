...
\caption{Microkernel first presented by \citeN{Mytkowicz:2009:WrongData}, showing bias to environment size. Note the clear spike at around 3200 bytes, where the number of cycles executed nearly doubles. Measuring performance in this configuration would give a biased result.}
\end{figure}
...
We later reproduced similar results on the previous ``Nehalem'', ``Sandy Bridge'' and ``Ivy Bridge'' CPUs, as well as an older Core2 Duo machine.
Each setup is described in Table~\ref{tab:machines}, and referenced throughout the paper. The compiler used is GCC 4.8.2-19ubuntu1 and the same binary is tested on all systems.
...
Older architectures such as Core do not have this particular event listed, however 4K aliasing is covered by a more general event:
\begin{describe}{{pad}}
...
Similar experiments were performed on each of the other experimental setups as well, all arriving at the same conclusions.
...
The biggest practical difference occur for Core 2, where a significantly different set of performance events were used.
In particular; load\_blocks.overlap_store indicating alias.
Notably, this event was also found to be significant in the original paper by \citeN{Mytkowicz:2009:WrongData}.
We were able to verify that partial address overlap was indeed occuring at the same time this event is spiking, and we draw the conclusion that address aliasing must be the underlying explanation.
However, this only explains part of what is going on in the Core setup. 
As shown in Figure~\ref{fig:motivation}, there seem to be yet another bias effect visible at every fourth environment increment.
...
Results for cycles executed and alias events were virtually identical, indicating that configuration of HTT either way is not impactful to our conclusions.
