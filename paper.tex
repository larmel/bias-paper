...
These counters such as these are obviously not indicative of any causal relationship, and are ommitted.
...
A heap allocator is responsible for managing dynamic memory, and ultimately assigns the actual addresses of heap allocated variables at run time.
...
The particular library used therefore constitutes an important part of the execution context, as linking to a different library, or a library with some alternative configuration, can impact heap addresses at run time.

\subsection{\texttt{malloc} will Alias by Default}
Acquiring dynamic memory at run time is usually done by calling \texttt{malloc}, which takes a number of bytes to allocate as input, and returns a pointer to that area.
A typical malloc implementation uses different strategies for how to allocate memory depending on the size of the request.
\begin{itemize}
  \item The ``regular'' heap area in virtual memory is used for smaller allocations. 
        ...
        The heap section starts at a relatively low address (close to the data section), and grows upwards.
  \item For larger allocations, instead of growing the regular heap it is typically faster to use anonymous memory mapping by calling the Posix function \texttt{mmap}.
      Memory mapped chunks are typically placed towards the upper end of the virtual address space, closer to the stack.
\end{itemize}
Addresses for smaller requests that are placed in the regular heap can look something like 0x16e30a0 or 0x1723020, while pointers returned by \texttt{mmap} can for example be 0x7f0318a8f010 or 0x7f03105d2010.
...
The page size is 4096 bytes, meaning two pointers returned by \texttt{mmap} will \emph{always} alias.\footnote{libc's version of malloc adds 16 bytes of medatada at the beginning, therefore every mmap'ed address ends with 0x010}


\subsection{Aligned Sequential Access}
Many functions operate on pairs of buffers in a ``sliding window'' fashion; reading from one and writing the result to another in some loop construct.
...
This effect is mentioned in the Intel optimization manual, exemplified by an aliasing copy routine.
...
The function from Listing \ref{lst:conv} is a hand crafted naive implementation of linear correlation, containing lots of interleaved loads and stores while iterating over two independent memory buffers (for simplicity, endpoints are not handled).
We show that the performance of this program greatly depends on the address alignment of each buffer, favoring memory addresses that are not closely aligned on the last 12 bits.
For heap allocated memory, this depends entirely on what addresses the heap allocator provides.
...
% Plot of cycles vs alias for estimated cost of O3 convolution
\pgfplotstableread{bin/conv-default-o3.estimate.dat}{\convolutiontable}
\begin{figure}[t]
...
% Table with detailed performance data, estimated for O3 (sidewaystable)
\begin{table*}[t]
  \caption{Most interesting performance counters for O3 convolution code, estimated cost accounting for constant overhead of heap allocation in main.}
  \label{tab:convstats}
  ...
  \pgfplotstabletypeset[
    ...
    int detect, % Output whole numbers for counter values
    col sep=comma,
    columns={Performance counter, Correlation, 0, 2, 4, 8, 16, 24},
    column type=r,
    columns/Performance counter/.style={
      string type, 
      column type=l,
    ...
    every head row/.style={
      output empty row,
      before row={\toprule
        Performance counter & $r$ & 0 & 2 & 4 & 8 & 16 & 24 \\
        },
      after row=\midrule,
    },
    every last row/.style={after row=\bottomrule}
  ]{bin/conv-default-o3.estimate.csv}
\end{table*}
