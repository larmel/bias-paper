...
The results show a high number of resource stalls when the spikes occur, indicated by high values for performance counters RESOURCE\_STALLS.ANY, CYCLE\_ACTIVITY.CYCLES\_LDM\_PENDING, CYCLE\_ACTIVITY.CYCLE\_NO\_EXECUTE and RESOURCE\_STALLS.RS.
We also find that LD\_BLOCKS\_PARTIAL.ADDRESS\_ALIAS correlates almost perfectly with cycle count, which can explain the high number of resource stalls with false dependency from aliasing.
...
\begin{figure}[t]
  \caption{Cycle and alias counts for different offsets between input and output arrays in Listing~\ref{lst:conv}. Offset 0 means equal 12 bit address suffix, which default behavior for mmap allocations. Array size $n=2^{20}$, compiled with -O3. }
  \label{fig:heapalias}
  \begin{tikzpicture}
    \begin{axis}[
        font = \small,
        xlabel=Offset in \lstinline!sizeof(float)! bytes,
...
Figure \ref{fig:heapalias} shows ...

Using malloc to allocate input and output float arrays of size n, they will alias if mmap is used internally.
For sufficiently large values of n, this will always be the case.
Compiling with optimization O3 and n=$2^{20}$, a very large number of alias events and resource stalls are generated compared to cycle count.
