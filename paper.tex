...
\caption{Microkernel first presented by \citeN{Mytkowicz:2009:WrongData}, showing bias to environment size. Note the clear spike around 3200 bytes, where the number of cycles executed nearly doubles. Measuring performance in this configuration would give a biased result.}
\end{figure}
...
Older architectures such as Core 2 does not have this particular event listed, however 4K aliasing is covered by a more general event:
\begin{describe}{{pad}}
...
To illustrate how address aliasing can cause bias, we revisit the example first presented in ``Producing Wrong Data Without Doing Anything Obviously Wrong!'' by \citeN{Mytkowicz:2009:WrongData}, a small C program listed in Figure~\ref{fig:motivation}. 
...
Bacause the memory disambiguation system does not handle address aliasing, the potential for out of order execution is limited~\cite[Page 2-20]{OptimizationManual}.
...
With Hyper-Threading we see smalls spikes like Core2, likely cache bank issues. % todo: fill in something here
