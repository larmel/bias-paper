...
\usepackage{tikz}
\usepackage{listings}
%\usepackage{authblk} % acm error, \affil
\usepackage{caption} % 
...
In this paper we identify one of the underlying mechanics resulting in measurement bias on modern Intel microarchitectures.
...
% In addition to environment variable size, we show how address aliasing can cause measurement bias from variations in dynamically linked heap allocation libraries.
We find that performance penalties from unfavorable memory address allocations can be more than double the cycle count, even for real world applications.
Furthermore, the dynamic memory allocation pattern for common libraries like libc tends to give \emph{worst case} performance by default for large allocations.
An understanding of address aliasing is necessary in order to achieve optimal performance, and we outline how developers can utilize this knowledge.
\end{abstract}

%\category{}{}{}
\terms{Measurement bias, Optimization}
\keywords{4K address aliasing, Intel ``Haswell'' architecture, heap allocators}
...
% Include preview of aliasing program early to provike interest, also lightly referenced in introduction/background
% Cite does not work in acm style
% Micro-kernel presented by Mytkowicz et al~\cite{Mytkowicz:2009:WrongData}, 
\begin{lstlisting}[float=t, language=C, caption={Micro-kernel succeptible to bias from variations to system environment variables.}, label={lst:loopkernel}, frame=lines]
static int i, j, k;
int main() {
    int g = 0, inc = 1;
    for (; g < 65536; g++) {
