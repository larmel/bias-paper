...
\twocolumn[
  \begin{@twocolumnfalse}
    ...
    % Motivation
    % Seemingly innocious properties of the execution context can cause subtle interactions with the low level hardware boundaries.
    % Because of the vast complexity of modern hardware, these effects are found to be unpredictable and difficult to deal with.
    % Programs are said to be \emph{biased} towards certain machines and configurations in various ways, .

    % Interaction with cache boundaries, branch prediction and other intricate details of the underlying hardware can make accurate performance measurements difficult. 

    % Problem statement
    In this paper we show how a lesser known optimization in the out-of-order execution engine can be a common cause of measurement bias on moderns Intel microarchitectures.

    % Approach
    Our discovery stems from an analysis of a small program that was presented as a curiosity in previous work. The program shows clear bias in that for certain sizes of system environment variables, the performance is significantly worse than average. 
    Using hardware performance counters we identify the source of bias to be resource stalls from falsely detected dependencies in the out-of-order execution pipeline, reported as 4K address aliasing.
    ...
    Using a constructed example, we find that performance penalties from unfavorable memory address allocations can be as high as double the cycle count.
    ...
    % Conclusions
    % In principle anything that can alter run time addresses of program data can potentially result in bias from aliasing.
    % We show that bias caused by address aliasing is in fact not completely unpredictable, and should be taken into account by optimizing 
    \end{abstract}
    \bigskip
