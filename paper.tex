...
\author{\IEEEauthorblockN{Lars Kirkholt Melhus, Rune Erlend Jensen}
\IEEEauthorblockA{Dept. of Computer and Information Science (IDI)\\
NTNU, Norway\\
lars.kirkholt.melhus@gmail.com, runeerle@idi.ntnu.no}
}
...
By analyzing hardware performance counters, we show how bias can occur from two external factors; size of environment variables, and characteristics of dynamically linked heap allocators.
...
This is especially important for heap allocations, as most allocators tend to produce worst case alignment by default; favoring page alignment.
...
The performance impact of unfavorable memory layouts can be significant, with as much as $2x$ speedup in already optimized code.
...
Seemingly innocious external factors---such as link ordering or contents of environment variables---have been shown to have potentially significant impacts on performance in real applications~\cite{Mytkowicz:2009:WrongData}.
...
Finally, in Section~\ref{sec:mitigation} we discuss some approaches to deal with aliasing effects.
...
Loads are issued \emph{speculatively}, based on a prediction on whether it will conflict with a previous store that is still not retired.
...
We see a significant change from the median in the number of alias events. 
...
Memory layout of static data is decided at compile time, and we find the addresses to be \texttt{\&i} = 0x60103c, \texttt{\&j} = 0x601040, and \texttt{\&k} = 0x601044 by reading the ELF symbol table.%\footnote{ELF symbol tables can be read using \texttt{readelf -s}}
...
Figure~\ref{fig:microkernel-all-archs} shows cycle count plots for ``Core'', ``Nehalem'' and ``Ivy Bridge'', revealing the same alias behavior.%\footnote{For these results we also pinned the process to a single core during measurements using \texttt{taskset}, which helps remove some of the random noise.}
...
We were only able to observe the effect on this particular architecture, and did not investigate the root cause.% is related to 4K aliasing.
...
Allocating and managing these buffers at startup takes a non-negligible amount of work, but the overhead can be masked by repeatedly invoking the convolution kernel in a loop, after allocating and initializing all the inputs.
...
For both for optimization levels 2 and 3, this is close to worst case performance.
...
The Intel optimization manual mentions this% in \emph{User/Source Coding Rule 8}
