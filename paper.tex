  ...
  \begin{flushleft}
    ...
    \begin{tikzpicture}
      \begin{axis}[
        ...
        \addplot[ycomb] table[x expr = \thisrowno{0}*16, y = cycles:u] \microkernelcoretwotable ;
      \end{axis}
    \end{tikzpicture}
    %}
  \end{flushleft}
...
\caption{Microkernel first presented by \citeN{Mytkowicz:2009:WrongData}, showing bias to environment size. Notice the clear spike at around 3200 bytes, where the number of cycles executed nearly doubles. Measuring performance in this configuration would give a biased result.}
\end{figure}
...
In Figure~\ref{fig:motivation}, we have reproduced a result originally presented by \citeN{Mytkowicz:2009:WrongData}, showing how the number of cycles executed for a simple program depends on environment variables.
...
% The compiler used is GCC 4.8.2-19ubuntu1, and the same binaries are tested on all systems.

\begin{table}
  \tbl{Experimental setups used\label{tab:machines}}{
  \begin{tabular}{|l|l|}
    \hline
...
To illustrate how address aliasing can cause bias, we revisit the example first presented in ``Producing Wrong Data Without Doing Anything Obviously Wrong!'' by \citeN{Mytkowicz:2009:WrongData}, a small C program reproduced here in Figure~\ref{fig:motivation}.
...
\pgfplotstableread{bin/microkernel-haswell-ht.dat}{\tblmicrokernelhaswellht}
\begin{figure}[t]
  \begin{tabular}{lr}
    \subfigure[Core]
    {\begin{tikzpicture}
      \begin{axis}[
          width=\textwidth/2.0, % Make it fit to text width side by side
          height = 4cm,
          font = \footnotesize,
          domain = 0:4096,
          xtick = {0,1024,...,4096},
    ...
    {\begin{tikzpicture}
      ...
      \end{axis}
      \label{subfig:microkernel-nehalem}
    \end{tikzpicture}} \\
    ...
    {\begin{tikzpicture}
      ...
      \end{axis}
      \label{subfig:microkernel-ivybridge}
    \end{tikzpicture}} &
    \subfigure[Haswell (HT)]
    {\begin{tikzpicture}
      \begin{axis}[
          width=\textwidth/2.0, % Make it fit to text width side by side
          height = 4cm,
        ...
        \addplot[ycomb] table[x expr = \thisrowno{0}*16, y = cycles:u] \tblmicrokernelhaswellht ;
      \end{axis}
      \label{subfig:microkernel-haswell-ht}
    \end{tikzpicture}}
  \end{tabular}
  \caption{\label{fig:microkernel-all-archs}Microkernel experiment on different architectures. Horizontal axis represents bytes added to environment, and vertical axis is number of cycles executed.}
\end{figure}

\subsection{Other Architectures}
While we show only the analysis for Haswell here, the same approach gave similar results for all CPUs we tested.
Figure~\ref{fig:microkernel-all-archs} shows cycle count plots for Core, Nehalem and Ivy Bridge as well, revealing very similar patterns.

There were some practical differences in obtaining these results, in particular slightly different sets of performance events supported on each architecture.
For the Core setup we use \emph{load\_blocks.overlap\_store} instead of \emph{load\_blocks\_partial.address\_alias} to indicate alias.
Notably, this event was also found to be significant in the original paper by \citeN{Mytkowicz:2009:WrongData}, where the authors also used a Core 2 processor.
...
However, this only explains part of what is going on in the Core setup. 
As shown in Figure~\ref{subfig:microkernel-core}, there seem to be yet another bias effect visible at every fourth environment increment.
...
Based on our results, and existing documentation on available performance events, it is reasonable to assume that the address alias issue exists for a wide range of Intel microarchitectures, going as far back as the ``Core'' platform.

\subsection{Hyper-Threading}
As described in Section~\ref{sec:methodology}, disabling support for Hyper-Threading on applicable architectures was done in order to avoid any potential interference while collecting data.
However, this begs the question if doing so might affect the existence of aliasing itself.
To verify this was not the case, we ran the experiment again on the Haswell setup with Hyper-Threading enabled.
The raw cycle count results are shown in Figure~\ref{subfig:microkernel-haswell-ht}.
During this experiment we pinned the process to a particular core using \texttt{taskset}, and ran \texttt{cat /dev/zero > /dev/null} to create some background noise on the other (shared) core.
While the results are more noisy, we see the same trend with respect to cycles executed and alias events spiking at a particular point.
This indicates that configuration of Hyper-Threading either way is not impactful to our conclusions, address aliasing is still present.
...
\begin{table}[t]
  \tbl{Addresses returned by different heap allocators when allocating pairs of equally sized buffers.\label{tab:mallocompare}}{
    \pgfplotstabletypeset[
      col sep=comma,
      string type,
